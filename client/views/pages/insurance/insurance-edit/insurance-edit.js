Template.insuranceEdit.onRendered( function(){
   ...
   $('#date_year .input-group.date').datepicker({
       autoclose: true,
       minViewMode: 1,
       format: 'mm/yyyy'
...
//    console.log("Current ID: ", currentId, Insurances.findOne({_id: currentId}));

//    populateInsuranceEditForm(Session.get('thisId'));

    var reactiveNode = $("#generalInformation1").get(0);
    reactiveNode.remove();

    var temp = this;
    Blaze.render(
      Template.generalInformation,
      temp.$('#generalInformation').get(0)
    );

    reactiveNode = $("#customerProducts1").get(0);
    reactiveNode.remove();

    Blaze.render(
      Template.customerProducts,
      temp.$('#customerProducts').get(0)
    );

    reactiveNode = $("#premiumZeroDep1").get(0);
    reactiveNode.remove();

    Blaze.render(
      Template.premiumZeroDep,
      temp.$('#premiumZeroDep').get(0)
    );

    reactiveNode = $("#payments1").get(0);
    reactiveNode.remove();
    ...
    reactiveNode = $("#previousPolicy1").get(0);
    reactiveNode.remove();

    Blaze.render(
      Template.previousPolicy,
      temp.$('#previousPolicy').get(0)
    );
...
//      Session.set("policyStatus", "Un-Booked");
  ...
  'submit': function(event, template) {
    ...
    insurance = {
      coverNote: template.find('#coverNote').value,
      ...
      status: policyStatus, //Session.get("policyStatus")
      policyFile: Session.get("fileURL")
    };

    Meteor.call('insurances.update', Session.get('thisId'), insurance, function(error) {
      if (error) {
        toastr.error("Error: ", error.reason);
        console.log('error', 'Error in processing login. ' + error.reason + '.');
        Meteor.call('logs.add', Meteor.user()._id, Meteor.user().profile.name, "Insurance", "Edit", "Error", error.reason, "");
      } else {
        toastr.success("Insurance Successfully Updated");
        Meteor.call('logs.add', Meteor.user()._id, Meteor.user().profile.name, "Insurance", "Edit", "Success", "", "");
